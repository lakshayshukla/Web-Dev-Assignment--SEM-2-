...
const EditSessionModal = ({ session, onClose }) => {
  ...
  const [selectedMood, setSelectedMood] = useState(session.mood || 'neutral')
  ...
  const handleSave = () => {
    ...
    dispatch(updateSession({ id: session.id, goal: goal.trim(), mood: selectedMood, tags: selectedTags }))
